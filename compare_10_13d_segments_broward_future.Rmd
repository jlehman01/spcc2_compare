---
title: "Compare SPCC 1.0 & 1.3d Refined Smoothed Segments - Future - Broward County"
output:
  html_document:
    df_print: paged
editor_options:
...
ref_area <- st_read("K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13b\\inputs\\Broward.gdb",
          layer = "RefArea", quiet = TRUE) #reference area (for clipping segments and plcc layer)

layername <- "smoothed_segments_broward_overlay"
...
tm_shape(plcc_join) + tm_lines(col = "plcc_improvement", 
                               ...
                               popup.vars = c("plccID", "SPCC", "PLCC", "class_13d"),
                               title.col = "SPCC-PLCC Improvement Category")


```

#### Table of Summarized SPCC 1.0 - SPCC 1.3d - PLCC Alighment
...
plcc_segment_names = c("PLCC ID", "SPCC 1.3d", "SPCC 1.0", 
                    "PLCC", "Improvement Category", "Linear Miles")

plcc_join %>%
  select(plccID, class_13d, SPCC, PLCC, plcc_improvement) %>% 
  mutate(linear_miles = as.numeric(st_length(.))/5280) %>%
  st_drop_geometry() %>% 
  filter(linear_miles > 0) %>%
