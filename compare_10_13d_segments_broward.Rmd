...
ref_area <- st_read("K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13b\\inputs\\Broward.gdb",
          layer = "RefArea") #reference area (for clipping segments and plcc layer)

layername <- "broward_refined_smoothed_segments_overlay"
...
plcc_join <- st_join(plcc_buffer, segments,
          largest = TRUE) %>% 
  st_drop_geometry() 

plcc_spcc_merged <- left_join(dplyr::select(plcc, plccID), plcc_join, by = "plccID")
