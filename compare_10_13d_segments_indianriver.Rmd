...
#adjust precision to accoutn for slight differences, and intersect

st_precision(plcc) <- 10
st_precision(segments) <- 10

plcc_spcc_intersection <- st_intersection(st_zm(segments), plcc) %>% 
  dplyr::select(class_base, class_13d, SPCC, PLCC)

tm_shape(plcc_spcc_intersection) + tm_lines(lwd = 2.5, col = "PLCC", 
                          palette = "viridis")


tm_shape(plcc) + tm_lines(col ="red", lwd = 2) +
  tm_shape(segments) + tm_lines(col ="blue", lwd = 3)
