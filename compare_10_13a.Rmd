...
The purpose of this page is to review SPCC version 1.3a and compare it to version 1.0. Version 1.0 represents the existing version of SPCC and provides a baseline for comparison. Version 1.3a includes updated data layers where new data is available in addition to refined measures for evaluating intersection density, building placement, and land use diversity. Further, version 1.3a includes modified weights for density, diversity, and design scores.
...
Version 1.3a includes the following changes from version 1.0.
...
# SPCC 1.0 and SPCC 1.3a Mapping: Detailed Analytic Land Use

```{r}

spcc10_gdb <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC\\Outputs\\Existing\\PBC\\Version4k.gdb"

spcc13a_gdb <- 'K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13\\outputs\\existing\\Version_2_1_3.gdb'

spcc10 <- as.raster(arc.raster(arc.open(paste0(spcc10_gdb, "\\DetAnalyticLU_FINAL"))))

spcc13a <- as.raster(arc.raster(arc.open(paste0(spcc13a_gdb, "\\DetAnalyticLU_FINAL_reclass"))))

crs(spcc10) <- crs(spcc13a)

diff <- overlay(spcc10, spcc13a, fun=function(a,b) return(a==b))

#calculate directional change

spcc10_reclass <- reclassify(spcc10, reclass_matrix)

spcc13a_reclass <- reclassify(spcc13a, reclass_matrix)

directional_change <- overlay(x = spcc10_reclass, y = spcc13a_reclass, fun = function(x, y) ifelse(x < y, 3,ifelse(x == y, 2,ifelse(x > y, 1, NA))))
...
map2 <- tm_shape(spcc13a) + tm_raster(style = "cat", palette = spcc_palette, labels = spcc_labels, title = "SPCC 1.3a") + tm_basemap("Esri.WorldImagery")
...
### Cross tabulation of SPCC 1.0 versus SPCC 1.3a

The table below reviews changes from SPCC version 1.0 to 1.3a in tabular form. The rows indicate the number of cells of each SPCC classification in version 1.0. The columns represent the number of cells in version 1.3a. This allows for a quick review of which categories are changing, and how they are shifting.

```{r}
crosstab(spcc10, spcc13a) %>% kbl() %>% 
...
The following series of maps provide a side-by-side comparison of how different input layers differ between versions 10. and 1.3a. The purpose of these maps is to review how the new refined measures compare to the old measures used in version 1.0.

## Building Placement

### Changes in Measure

The following maps review the new building placement input layers. In version 1.3a, a setback is estimated for each building. This measurement is computed with two datasets: building footprints and the street network. The center of the edge of a building that is nearest to any non-private street is is identified, and the distance from this point to the street centerline is calculated. This distance is then associated with the building.
...
spcc13a_bulding_placement <- as.raster(arc.raster(arc.open(paste0(spcc13a_gdb, "\\BE_03Design_01BldgPlcmt"))))

crs(spcc10_bulding_placement) <- crs(spcc13a_bulding_placement)
```
...
spcc_building_placement_2 <- tm_shape(spcc13a_bulding_placement) + 
  tm_raster(style = "fixed", title = "SPCC 1.3a Building Placement",
            breaks = c(0, 20, 75, 999999),
            palette = 'viridis') + tm_basemap("Esri.WorldImagery")
...
spcc13a_intden_gdb <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13\\inputs\\PBC.gdb\\intsect_den"


spcc10_intden <- as.raster(arc.raster(arc.open(spcc10_intden_gdb)))

spcc13a_intden <- as.raster(arc.raster(arc.open(spcc13a_intden_gdb)))

crs(spcc10_intden) <- crs(spcc13a_intden)
```
...
spcc_intden_2 <- tm_shape(spcc13a_intden) + 
  tm_raster(style = "fixed", title = "SPCC 1.3a Intersection Density",
...
In version 1.0, diversity is calculated as the number of different simplified land uses present in a 1/4 mile radius of an analytic land use cell. In version 1.3a, diversity is calculated using the Effective Number of Parties (ENP) approach. This approach weights More information about this methodology can be found [here](https://en.wikipedia.org/wiki/Effective_number_of_parties).
...
spcc13a_diversity_gdb <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13\\inputs\\PBC.gdb\\diversity_enp_v2"


spcc10_diversity <- as.raster(arc.raster(arc.open(spcc10_diversity_gdb)))

spcc13a_diversity <- as.raster(arc.raster(arc.open(spcc13a_diversity_gdb)))

crs(spcc10_diversity) <- crs(spcc13a_diversity)
```
...
spcc_diversity_2 <- tm_shape(spcc13a_diversity) + 
  tm_raster(style = "fixed", title = "SPCC 1.3a Diversity",
...
pbc_folder <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13\\configuration_files\\existing\\PBC\\"
global_folder <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13\\configuration_files\\existing\\D4\\"

pbc_files <- list.files(pbc_folder, pattern = ".json", full.names = TRUE)

read_jsons <- function(x) {
  x %>% 
    fromJSON(simplifyVector = TRUE)
 
}

test <- lapply(pbc_files, read_jsons)


pbc_folder %>% 
  map_df(~fromJSON, simplifyVector = TRUE)

fromJSON("K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13b\\configuration_files\\existing\\PBC\\BE_03Design_01BldgPlcmt.json", simplifyMatrix = TRUE) %>% 
  kbl()
...
data <- pbc_files %>%
       map_df(~fromJSON(file.path(pbc_folder, .)))
