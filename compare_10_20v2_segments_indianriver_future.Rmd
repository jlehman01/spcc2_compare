---
title: "Compare SPCC 1.0 & 2.0v2 Refined Smoothed Segments - Future - Indian River County"
output:
  html_document:
    df_print: paged
editor_options:
...
This website compares version 2.0v2 to version 1.0. Version 1.0 represents the future version of SPCC and provides a baseline for comparison. Version 2.0v2 includes updated data layers where new data is available in addition to refined measures for evaluating intersection density, building placement, and land use diversity.


# SPCC 1.0 and SPCC 2.0v2 Mapping: Refined Smoothed Segments
...
map3 <- tm_shape(class_change) + 
  tm_lines(col = "directional_change", style = "cat", 
           ...
           title.col = "SPCC 1.0 vs. 2.0v2 - Directional Change", 
...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v1 != PLCC~ "1.0 Aligns with PLCC, 2.0v2 Does not",
                SPCC != PLCC & class_20v1 != PLCC ~ "1.0 & 2.0v2 Do Not Align with PLCC",
                SPCC != PLCC & class_20v1 == PLCC ~ "2.0v2 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v1 == PLCC ~ "1.0 & 2.0v2 Align with PLCC"),
...
### SPCC 1.0 vs. SPCC 2.0v2

### Cross tabulation of SPCC 1.0 versus SPCC 2.0v2 by Linear Feet

The table below provides a tabular comparison of SPCC refined smoothed analytic versions 1.0 to 2.0v2. The rows represent linear feet of each classification under SPCC 1.0, and the columns represent liner feet per classification under SPCC 2.0v2.
...
crosstab %>% 
  ...
  add_header_above(c("SPCC 1.0", "SPCC 2.0v2" = kbl_col_num))
...
#### Table of Summarized SPCC 1.0 - SPCC 2.0v2 - PLCC Alignment
...
plcc_segment_names = c("FM_Num", "PLCC ID", "SPCC 2.0v2", "SPCC 1.0", 
