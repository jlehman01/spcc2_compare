---
title: "Compare SPCC 1.0 & 2.0v4 Refined Smoothed Segments - Future - Indian River County"
output:
  html_document:
    df_print: paged
editor_options:
...
segments <- st_read(gdb, layer = layername, quiet = TRUE) %>%
  ...
  mutate(difference_flag = if_else(
         ...
         class_20v3 = case_when(
           value_20v4 %in% c(1, 700) ~ "C1 - Natural",
           value_20v4 == 2 ~ "C2 - Rural",
           value_20v4 == 307 ~ "C3R - Suburban Residential",
           value_20v4 %in% c(330, 800, 801) ~ "C3C - Suburban Commercial",
           value_20v4 == 400 ~ "C4 - Urban General",
           value_20v4 == 500 ~ "C5 - Urban Center",
           value_20v4 == 600 ~ "C6 - Urban Core",
           value_20v4 %in% c(900, 901, 902) ~ "Special Districts"),
         color_20v3 = case_when(
           value_20v4 %in% c(1, 700) ~ "#267300",
           value_20v4 == 2 ~ "#D7C29E",
           value_20v4 == 307 ~ "#F5EE1C",
           value_20v4 %in% c(330, 800, 801) ~ "#FFFFD4",
           value_20v4 == 400 ~ "#FFCCCC",
           value_20v4 == 500 ~  "#E60000",
           value_20v4 == 600 ~ "#730000",
           value_20v4 %in% c(900, 901, 902) ~ "#0084A8"),
         ) %>% 
...
class_change <- segments %>% 
  filter(value_base < 900 & value_base != 700 & 
           value_20v4 < 900 & value_20v4 != 700) %>% 
  mutate(value_base_for_change = parse_number(class_base),
         value_20v4_for_change = parse_number(class_20v3),
         directional_change = case_when(
           value_base_for_change < value_20v4_for_change ~ "class increase",
           value_base_for_change > value_20v4_for_change ~ "class decrease",
           value_base_for_change == value_20v4_for_change ~ "no change"))
...
map3 <- tm_shape(class_change) + 
  tm_lines(col = "directional_change", style = "cat", 
           ...
           popup.vars = c("value_base", "value_20v4")) +
    tm_shape(ref_area) + tm_borders(col = "#a80084") +
...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v3 != PLCC~ "1.0 Aligns with PLCC, 2.0v4 Does not",
                SPCC != PLCC & class_20v3 != PLCC ~ "1.0 & 2.0v4 Do Not Align with PLCC",
                SPCC != PLCC & class_20v3 == PLCC ~ "2.0v4 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v3 == PLCC ~ "1.0 & 2.0v4 Align with PLCC"),
         spcc10_overall = case_when(
                SPCC == PLCC ~ "SPCC 1.0 Aligns with PLCC"),
         spcc20v3_overall = case_when(
                class_20v3 == PLCC ~ "SPCC 2.0v4 Aligns with PLCC")
         ) %>% 
...
The table below provides a tabular comparison of SPCC refined smoothed analytic versions 1.0 to 2.0v4. The rows represent linear feet of each classification under SPCC 1.0, and the columns represent liner feet per classification under SPCC 2.0v4.
