---
title: "Compare SPCC 1.0 & 2.0v1 Refined Smoothed Segments - Existing - St. Lucie County"
output:
  html_document:
    df_print: paged
editor_options:
...
class_change <- segments %>% 
  ...
  mutate(value_base_for_change = parse_number(class_base),
         value_20v1_for_change = parse_number(class_20v13),
...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v13 != PLCC~ "1.0 Aligns with PLCC, 2.0v1 Does not",
                SPCC != PLCC & class_20v13 != PLCC ~ "1.0 & 2.0v1 Do Not Align with PLCC",
                SPCC != PLCC & class_20v13 == PLCC ~ "2.0v1 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v13 == PLCC ~ "1.0 & 2.0v1 Align with PLCC"),
         spcc10_overall = case_when(
                SPCC == PLCC ~ "SPCC 1.0 Aligns with PLCC"),
         spcc20v13_overall = case_when(
                class_20v13 == PLCC ~ "SPCC 2.0v1 Aligns with PLCC")
         ) %>% 
