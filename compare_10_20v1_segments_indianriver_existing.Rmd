---
title: "Compare SPCC 1.0 & 2.0v1 Refined Smoothed Segments - Existing - Indian River County"
output:
  html_document:
    df_print: paged
editor_options:
...
This website compares version 2.0v1 to version 1.0. Version 1.0 represents the future version of SPCC and provides a baseline for comparison. Version 2.0v1 includes updated data layers where new data is available in addition to refined measures for evaluating intersection density, building placement, and land use diversity.


# SPCC 1.0 and SPCC 2.0v1 Mapping: Refined Smoothed Segments
...
gdb <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v20_final\\outputs\\existing\\v20final_SegmentSmoothing.gdb"
...
class_change <- segments %>% 
  ...
  mutate(value_base_for_change = parse_number(class_base),
         value_20v1_for_change = parse_number(class_20v3),
         directional_change = case_when(
           value_base_for_change < value_20v1_for_change ~ "class increase",
           value_base_for_change > value_20v1_for_change ~ "class decrease",
           value_base_for_change == value_20v1_for_change ~ "no change"))
...
map2 <- tm_shape(segments) + 
  ...
  tm_add_legend(type = "fill", labels = legend_20v3_palette$class_20v3, 
                ...
                title = "SPCC 2.0v1 Refined Smoothed") +
  tm_shape(ref_area) + tm_borders(col = "#a80084") +
  tm_basemap("Esri.WorldImagery")
...
map3 <- tm_shape(class_change) + 
  tm_lines(col = "directional_change", style = "cat", 
           ...
           title.col = "SPCC 1.0 vs. 2.0v1 - Directional Change", 
           popup.vars = c("value_base", "value_20v1")) +
    tm_shape(ref_area) + tm_borders(col = "#a80084") +
...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v3 != PLCC~ "1.0 Aligns with PLCC, 2.0v1 Does not",
                SPCC != PLCC & class_20v3 != PLCC ~ "1.0 & 2.0v1 Do Not Align with PLCC",
                SPCC != PLCC & class_20v3 == PLCC ~ "2.0v1 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v3 == PLCC ~ "1.0 & 2.0v1 Align with PLCC"),
         spcc10_overall = case_when(
                SPCC == PLCC ~ "SPCC 1.0 Aligns with PLCC"),
         spcc20v3_overall = case_when(
                class_20v3 == PLCC ~ "SPCC 2.0v1 Aligns with PLCC")
         ) %>% 
...
### SPCC 1.0 vs. SPCC 2.0v1

### Cross tabulation of SPCC 1.0 versus SPCC 2.0v1 by Linear Feet

The table below provides a tabular comparison of SPCC refined smoothed analytic versions 1.0 to 2.0v1. The rows represent linear feet of each classification under SPCC 1.0, and the columns represent liner feet per classification under SPCC 2.0v1.
...
crosstab %>% 
  ...
  add_header_above(c("SPCC 1.0", "SPCC 2.0v1" = kbl_col_num))
...
#### Table of Summarized SPCC 1.0 - SPCC 2.0v1 - PLCC Alignment
...
plcc_segment_names = c("FM_Num", "PLCC ID", "SPCC 2.0v1", "SPCC 1.0", 
