...
Version 1.3f uses a new approach to differentiating C-1 and C-2. To quality as C-1, a grid cell must now must have density-diversity-design characteristics below a C-3 AND have some level protection as a park or open space. 1.3f also includes a C-5 overlay that provides an alternative path for C-4 to become C-5 if there is a cluster of civic uses and employment density thresholds are met for C-5.
