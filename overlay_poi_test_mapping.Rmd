...
### Output C5 Overlay Areas

```{r}
c5_overlay <- points_with_geocode %>%
  dplyr::filter(pct_c5_emp >= 20) %>% 
  st_buffer(1320) %>% 
  transmute(future = 0,
         SD_type = 5,
         Flag_1 = 0)

#st_write(c5_overlay, "K:/Projects/D4_CSHnetwork/Features/LU/SPCC_20/data_prep/cleaned_data/c5_overlay/c5_overlay.shp")

tm_shape(c5_overlay) + tm_fill(col = "#481567FF")
```
