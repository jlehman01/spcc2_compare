---
title: "Compare SPCC 1.0 & 2.0v3 Refined Smoothed Segments - Future - St. Lucie County"
output:
  html_document:
    df_print: paged
editor_options:
...
This website compares version 2.0v3 to version 1.0. Version 1.0 represents the future version of SPCC and provides a baseline for comparison. Version 2.0v3 includes updated data layers where new data is available in addition to refined measures for evaluating intersection density, building placement, and land use diversity.


# SPCC 1.0 and SPCC 2.0v3 Mapping: Refined Smoothed Segments
...
segments <- st_read(gdb, layer = layername, quiet = TRUE) %>%
  ...
  mutate(difference_flag = if_else(
         ...
         class_20v13 = case_when(
           value_20v13 %in% c(1, 700) ~ "C1 - Natural",
           value_20v13 == 2 ~ "C2 - Rural",
           value_20v13 == 307 ~ "C3R - Suburban Residential",
           value_20v13 %in% c(330, 800, 801) ~ "C3C - Suburban Commercial",
           value_20v13 == 400 ~ "C4 - Urban General",
           value_20v13 == 500 ~ "C5 - Urban Center",
           value_20v13 == 600 ~ "C6 - Urban Core",
           value_20v13 %in% c(900, 901, 902) ~ "Special Districts"),
         color_20v13 = case_when(
           value_20v13 %in% c(1, 700) ~ "#267300",
           value_20v13 == 2 ~ "#D7C29E",
           value_20v13 == 307 ~ "#F5EE1C",
           value_20v13 %in% c(330, 800, 801) ~ "#FFFFD4",
           value_20v13 == 400 ~ "#FFCCCC",
           value_20v13 == 500 ~  "#E60000",
           value_20v13 == 600 ~ "#730000",
           value_20v13 %in% c(900, 901, 902) ~ "#0084A8"),
         ) %>% 
...
class_change <- segments %>% 
  filter(value_base < 900 & value_base != 700 & 
           value_20v13 < 900 & value_20v13 != 700) %>% 
  mutate(value_base_for_change = parse_number(class_base),
         value_20v13_for_change = parse_number(class_20v13),
         directional_change = case_when(
           value_base_for_change < value_20v13_for_change ~ "class increase",
           value_base_for_change > value_20v13_for_change ~ "class decrease",
           value_base_for_change == value_20v13_for_change ~ "no change"))
...
legend_20v13_palette  <- segments %>%
  st_drop_geometry() %>% 
  distinct(color_20v13, class_20v13) %>% 
  drop_na() %>% 
  arrange(class_20v13)
...
map2 <- tm_shape(segments) + 
  tm_lines(col = "color_20v13", lwd = 2.5, 
           showNA = F) +
  tm_add_legend(type = "fill", labels = legend_20v13_palette$class_20v13, 
                col = legend_20v13_palette$color_20v13,
                title = "SPCC 2.0v3 Refined Smoothed") +
  tm_shape(ref_area) + tm_borders(col = "#a80084") +
  tm_basemap("Esri.WorldImagery")
...
map3 <- tm_shape(class_change) + 
  tm_lines(col = "directional_change", style = "cat", 
           ...
           title.col = "SPCC 1.0 vs. 2.0v3 - Directional Change", 
           popup.vars = c("value_base", "value_20v13")) +
    tm_shape(ref_area) + tm_borders(col = "#a80084") +
...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v13 != PLCC~ "1.0 Aligns with PLCC, 2.0v3 Does not",
                SPCC != PLCC & class_20v13 != PLCC ~ "1.0 & 2.0v3 Do Not Align with PLCC",
                SPCC != PLCC & class_20v13 == PLCC ~ "2.0v3 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v13 == PLCC ~ "1.0 & 2.0v3 Align with PLCC"),
         spcc10_overall = case_when(
                SPCC == PLCC ~ "SPCC 1.0 Aligns with PLCC"),
         spcc20v13_overall = case_when(
                class_20v13 == PLCC ~ "SPCC 2.0v3 Aligns with PLCC")
         ) %>% 
  filter(segment_ft > 11)

plcc_spcc_map <- tm_shape(plcc_join) + tm_lines(col = "plcc_improvement", 
                               palette = "-viridis", lwd = 4,
                               popup.vars = c("FM_Num", "plccID", "SPCC", "PLCC", "class_20v13"),
...
### SPCC 1.0 vs. SPCC 2.0v3

### Cross tabulation of SPCC 1.0 versus SPCC 2.0v3 by Linear Feet

The table below provides a tabular comparison of SPCC refined smoothed analytic versions 1.0 to 2.0v3c. The rows represent linear feet of each classification under SPCC 1.0, and the columns represent liner feet per classification under SPCC 2.0v3.
...
crosstab <- segments %>%
  st_drop_geometry() %>% 
  drop_na() %>% 
  group_by(class_base, class_20v13) %>% 
  arrange(class_base) %>% 
  summarise(segment_ft = sum(segment_ft)) %>% 
  arrange(class_20v13) %>% 
  pivot_wider(names_from = class_20v13, values_from = segment_ft) 
...
crosstab %>% 
  ...
  add_header_above(c("SPCC 1.0", "SPCC 2.0v3" = kbl_col_num))
...
#### Table of Summarized SPCC 1.0 - SPCC 2.0v3 - PLCC Alignment
...
comparison <- plcc_join %>% 
  ...
  select(linear_miles, total_linear_miles, spcc10_overall, spcc20v13_overall)
...
comparison %>%
  select(linear_miles, total_linear_miles, spcc20v13_overall) %>% 
  group_by(spcc20v13_overall) %>% 
  summarise(spcc_20v13_miles = sum(linear_miles),
            total_linear_miles = first(total_linear_miles),
            spcc_20v13_percent = spcc_20v13_miles / total_linear_miles,
            spcc_20v13_segment_count = n()) %>% 
  drop_na(spcc20v13_overall) %>% 
  mutate(spcc_20v13_percent = pct_format(spcc_20v13_percent)) %>% 
  kbl(digits = 3, col.names = comparison_names) %>% 
  kable_paper("hover", full_width = F)
```
...
intersection <- st_intersection(plcc_join, access_class_buffer) %>% 
  ...
  dplyr::select(ROADID, SPCC, class_20v13, PLCC, plcc_improvement, ACCESS_CLA, length)
...
plcc_segment_names = c("FM_Num", "PLCC ID", "SPCC 2.0v3", "SPCC 1.0", 
                    "PLCC", "Improvement Category", "Linear Miles")

plcc_join %>%
  select(FM_Num, plccID, class_20v13, SPCC, PLCC, plcc_improvement) %>% 
  mutate(linear_miles = as.numeric(st_length(.))/5280) %>%
  st_drop_geometry() %>% 
  filter(linear_miles > 0) %>%
