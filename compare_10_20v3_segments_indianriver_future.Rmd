---
title: "Compare SPCC 1.0 & 2.0v3 Refined Smoothed Segments - Future - Indian River County"
output:
  html_document:
    df_print: paged
editor_options:
...
ref_area <- st_read("K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v20_final\\inputs\\IndianRiver.gdb",
...
segments <- st_read(gdb, layer = layername, quiet = TRUE) %>%
  ...
  mutate(difference_flag = if_else(
         ...
         class_20v3 = case_when(
           value_20v3 %in% c(1, 700) ~ "C1 - Natural",
           value_20v3 == 2 ~ "C2 - Rural",
           value_20v3 == 307 ~ "C3R - Suburban Residential",
           value_20v3 %in% c(330, 800, 801) ~ "C3C - Suburban Commercial",
           value_20v3 == 400 ~ "C4 - Urban General",
           value_20v3 == 500 ~ "C5 - Urban Center",
           value_20v3 == 600 ~ "C6 - Urban Core",
           value_20v3 %in% c(900, 901, 902) ~ "Special Districts"),
         color_20v3 = case_when(
           value_20v3 %in% c(1, 700) ~ "#267300",
           value_20v3 == 2 ~ "#D7C29E",
           value_20v3 == 307 ~ "#F5EE1C",
           value_20v3 %in% c(330, 800, 801) ~ "#FFFFD4",
           value_20v3 == 400 ~ "#FFCCCC",
           value_20v3 == 500 ~  "#E60000",
           value_20v3 == 600 ~ "#730000",
           value_20v3 %in% c(900, 901, 902) ~ "#0084A8"),
         ) %>% 
...
class_change <- segments %>% 
  filter(value_base < 900 & value_base != 700 & 
           value_20v3 < 900 & value_20v3 != 700) %>% 
  mutate(value_base_for_change = parse_number(class_base),
         value_20v3_for_change = parse_number(class_20v3),
         directional_change = case_when(
           value_base_for_change < value_20v3_for_change ~ "class increase",
           value_base_for_change > value_20v3_for_change ~ "class decrease",
           value_base_for_change == value_20v3_for_change ~ "no change"))
...
legend_20v3_palette  <- segments %>%
  st_drop_geometry() %>% 
  distinct(color_20v3, class_20v3) %>% 
  drop_na() %>% 
  arrange(class_20v3)
...
map2 <- tm_shape(segments) + 
  tm_lines(col = "color_20v3", lwd = 2.5, 
           showNA = F) +
  tm_add_legend(type = "fill", labels = legend_20v3_palette$class_20v3, 
                col = legend_20v3_palette$color_20v3,
...
map3 <- tm_shape(class_change) + 
  tm_lines(col = "directional_change", style = "cat", 
           ...
           popup.vars = c("value_base", "value_20v3")) +
    tm_shape(ref_area) + tm_borders(col = "#a80084") +
...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v3 != PLCC~ "1.0 Aligns with PLCC, 2.0v3 Does not",
                SPCC != PLCC & class_20v3 != PLCC ~ "1.0 & 2.0v3 Do Not Align with PLCC",
                SPCC != PLCC & class_20v3 == PLCC ~ "2.0v3 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v3 == PLCC ~ "1.0 & 2.0v3 Align with PLCC"),
         spcc10_overall = case_when(
                SPCC == PLCC ~ "SPCC 1.0 Aligns with PLCC"),
         spcc20v3_overall = case_when(
                class_20v3 == PLCC ~ "SPCC 2.0v3 Aligns with PLCC")
         ) %>% 
  filter(segment_ft > 11)

plcc_spcc_map <- tm_shape(plcc_join) + tm_lines(col = "plcc_improvement", 
                               palette = "-viridis", lwd = 4,
                               popup.vars = c("FM_Num", "plccID", "SPCC", "PLCC", "class_20v3"),
...
The table below provides a tabular comparison of SPCC refined smoothed analytic versions 1.0 to 2.0v3. The rows represent linear feet of each classification under SPCC 1.0, and the columns represent liner feet per classification under SPCC 2.0v3.
...
crosstab <- segments %>%
  st_drop_geometry() %>% 
  drop_na() %>% 
  group_by(class_base, class_20v3) %>% 
  arrange(class_base) %>% 
  summarise(segment_ft = sum(segment_ft)) %>% 
  arrange(class_20v3) %>% 
  pivot_wider(names_from = class_20v3, values_from = segment_ft) 
...
comparison <- plcc_join %>% 
  ...
  select(linear_miles, total_linear_miles, spcc10_overall, spcc20v3_overall)
...
comparison %>%
  select(linear_miles, total_linear_miles, spcc20v3_overall) %>% 
  group_by(spcc20v3_overall) %>% 
  summarise(spcc_20v3_miles = sum(linear_miles),
            total_linear_miles = first(total_linear_miles),
            spcc_20v3_percent = spcc_20v3_miles / total_linear_miles,
            spcc_20v3_segment_count = n()) %>% 
  drop_na(spcc20v3_overall) %>% 
  mutate(spcc_20v3_percent = pct_format(spcc_20v3_percent)) %>% 
  kbl(digits = 3, col.names = comparison_names) %>% 
  kable_paper("hover", full_width = F)
```
...
intersection <- st_intersection(plcc_join, access_class_buffer) %>% 
  ...
  dplyr::select(ROADID, SPCC, class_20v3, PLCC, plcc_improvement, ACCESS_CLA, length)
...
plcc_join %>%
  select(FM_Num, plccID, class_20v3, SPCC, PLCC, plcc_improvement) %>% 
  mutate(linear_miles = as.numeric(st_length(.))/5280) %>%
  st_drop_geometry() %>% 
  filter(linear_miles > 0) %>%
