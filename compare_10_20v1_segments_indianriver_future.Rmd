...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v1 != PLCC~ "1.0 Aligns with PLCC, 1.3 Does not",
                SPCC != PLCC & class_20v1 != PLCC ~ "1.0 & 1.3 Do Not Align with PLCC",
                SPCC != PLCC & class_20v1 == PLCC ~ "1.3 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v1 == PLCC ~ "1.0 & 1.3 Align with PLCC"),
...
The table below provides a tabular comparison of SPCC refined smoothed analytic versions 1.0 to 1.3c. The rows represent linear feet of each classification under SPCC 1.0, and the columns represent liner feet per classification under SPCC 2.0v1.
