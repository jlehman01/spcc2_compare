---
title: "Compare SPCC 1.0 & 2.0v4 Refined Smoothed Segments - Future - St. Lucie County"
output:
  html_document:
    df_print: paged
editor_options:
...
This website compares version 2.0v4 to version 1.0. Version 1.0 represents the future version of SPCC and provides a baseline for comparison. Version 2.0v4 includes updated data layers where new data is available in addition to refined measures for evaluating intersection density, building placement, and land use diversity.


# SPCC 1.0 and SPCC 2.0v4 Mapping: Refined Smoothed Segments
...
segments <- st_read(gdb, layer = layername, quiet = TRUE) %>%
  ...
  mutate(difference_flag = if_else(
         ...
         class_20v13 = case_when(
           value_20v14 %in% c(1, 700) ~ "C1 - Natural",
           value_20v14 == 2 ~ "C2 - Rural",
           value_20v14 == 307 ~ "C3R - Suburban Residential",
           value_20v14 %in% c(330, 800, 801) ~ "C3C - Suburban Commercial",
           value_20v14 == 400 ~ "C4 - Urban General",
           value_20v14 == 500 ~ "C5 - Urban Center",
           value_20v14 == 600 ~ "C6 - Urban Core",
           value_20v14 %in% c(900, 901, 902) ~ "Special Districts"),
         color_20v13 = case_when(
           value_20v14 %in% c(1, 700) ~ "#267300",
           value_20v14 == 2 ~ "#D7C29E",
           value_20v14 == 307 ~ "#F5EE1C",
           value_20v14 %in% c(330, 800, 801) ~ "#FFFFD4",
           value_20v14 == 400 ~ "#FFCCCC",
           value_20v14 == 500 ~  "#E60000",
           value_20v14 == 600 ~ "#730000",
           value_20v14 %in% c(900, 901, 902) ~ "#0084A8"),
         ) %>% 
...
class_change <- segments %>% 
  filter(value_base < 900 & value_base != 700 & 
           value_20v14 < 900 & value_20v14 != 700) %>% 
  mutate(value_base_for_change = parse_number(class_base),
         value_20v14_for_change = parse_number(class_20v13),
         directional_change = case_when(
           value_base_for_change < value_20v14_for_change ~ "class increase",
           value_base_for_change > value_20v14_for_change ~ "class decrease",
           value_base_for_change == value_20v14_for_change ~ "no change"))
...
map2 <- tm_shape(segments) + 
  ...
  tm_add_legend(type = "fill", labels = legend_20v13_palette$class_20v13, 
                ...
                title = "SPCC 2.0v4 Refined Smoothed") +
  tm_shape(ref_area) + tm_borders(col = "#a80084") +
  tm_basemap("Esri.WorldImagery")
...
map3 <- tm_shape(class_change) + 
  tm_lines(col = "directional_change", style = "cat", 
           ...
           title.col = "SPCC 1.0 vs. 2.0v4 - Directional Change", 
           popup.vars = c("value_base", "value_20v14")) +
    tm_shape(ref_area) + tm_borders(col = "#a80084") +
...
plcc_join <- plcc_join %>% 
  mutate(segment_ft = as.numeric(st_length(.)),
         plcc_improvement = case_when(
                SPCC == PLCC & class_20v13 != PLCC~ "1.0 Aligns with PLCC, 2.0v4 Does not",
                SPCC != PLCC & class_20v13 != PLCC ~ "1.0 & 2.0v4 Do Not Align with PLCC",
                SPCC != PLCC & class_20v13 == PLCC ~ "2.0v4 Aligns with PLCC, 1.0 Does not",
                SPCC == PLCC & class_20v13 == PLCC ~ "1.0 & 2.0v4 Align with PLCC"),
         spcc10_overall = case_when(
                SPCC == PLCC ~ "SPCC 1.0 Aligns with PLCC"),
         spcc20v13_overall = case_when(
                class_20v13 == PLCC ~ "SPCC 2.0v4 Aligns with PLCC")
         ) %>% 
...
### SPCC 1.0 vs. SPCC 2.0v4

### Cross tabulation of SPCC 1.0 versus SPCC 2.0v4 by Linear Feet

The table below provides a tabular comparison of SPCC refined smoothed analytic versions 1.0 to 2.0v4c. The rows represent linear feet of each classification under SPCC 1.0, and the columns represent liner feet per classification under SPCC 2.0v4.
...
crosstab %>% 
  ...
  add_header_above(c("SPCC 1.0", "SPCC 2.0v4" = kbl_col_num))
...
#### Table of Summarized SPCC 1.0 - SPCC 2.0v4 - PLCC Alignment
...
plcc_segment_names = c("FM_Num", "PLCC ID", "SPCC 2.0v4", "SPCC 1.0", 
