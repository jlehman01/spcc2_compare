---
title: "Compare SPCC 1.0 & 1.3d - Indian River County"
output:
  html_document:
    df_print: paged
editor_options:
...
The purpose of this page is to review SPCC version 1.3d and compare it to version 1.0. Version 1.0 represents the existing version of SPCC and provides a baseline for comparison. Version 1.3d includes updated data layers where new data is available in addition to refined measures for evaluating intersection density, building placement, and land use diversity. Further, version 1.3d includes modified weights for density, diversity, and design scores. 1.3d differs from 1.3b in that the building setback search radius was changed from 1320 feet to 250 feet.
...
###PARAMETERS###
spcc10_gdb <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC\\Outputs\\Existing\\IndianRiver\\Version4k.gdb"

spcc13c_gdb <- 'K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13b\\outputs\\existing\\v13d_IndianRiver.gdb'

#SPCC 1.0 uses output location, as int den was generated from points.
spcc10_intden_input_gdb <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC\\Outputs\\Existing\\IndianRiver\\Version4k.gdb\\BE_03Design_02IntDens"

spcc13_intden_input_gdb <- "K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13b\\inputs\\D4_General.gdb\\intsect_den"

spcc10_diversity_input_gdb <- 'K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC\\Outputs\\Existing\\IndianRiver\\Version4k.gdb\\BE_02Diversity_01LU_count'

spcc13c_input_gdb <- 'K:\\Projects\\D4_CSHnetwork\\Features\\LU\\SPCC_20\\spcc_v2_13b\\inputs\\IndianRiver.gdb'
...
spcc13c <- as.raster(arc.raster(arc.open(paste0(spcc13c_gdb, "\\DetAnalyticLU_FINAL")))) %>% 
  reclassify(remove_0_reclass)
...
map2 <- tm_shape(spcc13c) + tm_raster(style = "cat", palette = spcc_palette, labels = spcc_labels, title = "SPCC 1.3d") + tm_basemap("Esri.WorldImagery")
...
```{r}
crosstab <- crosstab(spcc10, spcc13c) 
...
spcc13c_intden_gdb <- spcc13_intden_input_gdb
...
spcc13c_intden <- crop(spcc13c_intden, spcc10_intden)
```
...
spcc13c_diversity_gdb <- paste0(spcc13c_input_gdb, "\\diversity_enp")
...
spcc_diversity_2 <- tm_shape(spcc13c_diversity) + 
  tm_raster(style = "fixed", title = "SPCC 1.3d Diversity",
